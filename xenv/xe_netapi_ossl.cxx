...
#include <Communicatio-Linux/OpenSSLDL.h>
...
namespace Engine
{
	namespace XE
	{
		...
		{
			ESSE::oref<ESSE::OpenSSL::OSSLAPI> _api;
			ESSE::OpenSSL::SSL_CTX _ssl_ctx;
		public:
			OpenSSLContextLite(ESSE::OpenSSL::OSSLAPI * api) : _api(api), _ssl_ctx(0) {}
			virtual ~OpenSSLContextLite(void) override { if (_ssl_ctx) _api->SSL_CTX_free(_ssl_ctx); }
			void SetContext(ESSE::OpenSSL::SSL_CTX ssl_ctx) noexcept { if (_ssl_ctx) _api->SSL_CTX_free(_ssl_ctx); _ssl_ctx = ssl_ctx; if (_ssl_ctx) _api->SSL_CTX_up_ref(_ssl_ctx); }
			ESSE::OpenSSL::SSL_CTX GetContext(void) noexcept { return _ssl_ctx; }
			ESSE::OpenSSL::OSSLAPI * GetAPI(void) const noexcept { return _api; }
		};
		class OpenSSLContext : public IOpenSSLEventHandler
		{
			ESSE::oref<ESSE::OpenSSL::OSSLAPI> _api;
			ESSE::OpenSSL::SSL_CTX _ssl_ctx;
			ESSE::OpenSSL::SSL _ssl;
			ESSE::OpenSSL::BIO _bio;
			bool _connection_alive, _eos_sent;
			uint _send_eos_mode;
		public:
			OpenSSLContext(ESSE::OpenSSL::OSSLAPI * api) : _api(api), _ssl_ctx(0), _ssl(0), _bio(0), _connection_alive(true), _send_eos_mode(0), _eos_sent(false) {}
			...
			void InitializeSecurity(NetworkSecurityDescriptor & sec, ErrorContext & ectx) noexcept
			{
				...
				if (sec.IgnoreSecurity) _api->SSL_CTX_set_verify(_ssl_ctx, SSL_VERIFY_NONE, NULL);
				else _api->SSL_CTX_set_verify(_ssl_ctx, SSL_VERIFY_PEER, NULL);
				if (sec.Certificate) {
					auto bio = _api->BIO_new_mem_buf(sec.Certificate->GetBuffer(), sec.Certificate->Length());
				...
				if (sec.Domain.Length()) {
					...
					if (!_api->SSL_set_tlsext_host_name(_ssl, utf8)) throw OutOfMemoryException();
					if (!_api->SSL_set1_host(_ssl, utf8)) throw OutOfMemoryException();
				}
				XE_TRY_OUTRO()
			}
		...
		public:
			...
			virtual void ConnectB(NetworkAddress * address, NetworkSecurityDescriptor & sec, ErrorContext * error, IDispatchTask * hdlr, ErrorContext & ectx) noexcept 
			{
				...
				auto api = ESSE::OpenSSL::OSSLAPI::Query();
				_unix = CreateNetworkChannel(address);
				_ossl_ctx = new OpenSSLContext(api);
		...
		public:
			...
			virtual void BindB(NetworkAddress * address, NetworkIdentityDescriptor & idesc, ErrorContext & ectx) noexcept override
			{
				...
				_ossl_ctx = new OpenSSLContextLite(api);
				...
				auto bio = api->BIO_new_mem_buf(idesc.Data->GetBuffer(), idesc.Data->Length());
				if (!bio) throw OutOfMemoryException();
				auto pkcs12 = api->d2i_PKCS12_bio(bio, 0);
				api->BIO_free(bio);
				if (!pkcs12) throw InvalidFormatException();
				ESSE::OpenSSL::EVP_PKEY key = 0;
				ESSE::OpenSSL::X509 cert = 0;
				ESSE::OpenSSL::STACK_OF_X509 chain = 0;
			...
			virtual void Accept(int limit, ErrorContext * error, SafePointer<INetworkChannel> * channel, SafePointer<NetworkAddress> * address, IDispatchTask * hdlr, ErrorContext & ectx) noexcept override
			{
				...
				auto internal_accept = CreateStructuredTask< ErrorContext, SafePointer<INetworkChannel> >([error, channel, address, handler, ossl = _ossl_ctx](const ErrorContext & status, SafePointer<INetworkChannel> & inner) {
					if (status.error_code) {
						...
						try {
							full_context = new OpenSSLContext(ossl->GetAPI());
