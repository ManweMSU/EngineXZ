...
namespace Engine
{
	...
	uint64 ImmutableString::GeneralizedToUInt64(int dfrom, int dto, const ImmutableString & digits, bool case_sensitive) const
	{
		...
		for (int i = dfrom; i < dto; i++) {
			...
			for (int j = 0; j < digits.Length(); j++) {
				compare[0] = digits[j];
				if ((case_sensitive && StringCompare(input, compare) == 0) || (!case_sensitive && Compare(input, compare) == 0)) { dn = j; break; }
			}
	...
	{
		if (Compare(*this, L"true") == 0 || Compare(*this, L"1") == 0) return true;
		else if (Compare(*this, L"false") == 0 || Compare(*this, L"0") == 0 || Length() == 0) return false;
