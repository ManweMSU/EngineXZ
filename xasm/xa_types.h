...
namespace Engine
{
	namespace XA
	{
		...
		enum ReferenceTransforms : uint32
		{
			...
			TransformCryptAesDecECB	= 0x0303, // 4 arguments - a state (IV, unused); buffer (r/w); an integer length; a key; retval - no type
			TransformCryptAesEncCBC	= 0x0304, // 4 arguments - a state (IV, r/w); buffer (r/w); an integer length; a key; retval - no type
