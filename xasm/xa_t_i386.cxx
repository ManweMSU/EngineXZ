	...
	namespace XA
	{
		namespace i386
		{
			...
			class EncoderContext
			{
				...
				int _allocate_temporary(const ObjectSize & size, int * ebp_offs = 0)
				{
					...
					if (size == 8) {
						...
						if (disp->flags & DispositionPointer) {
							*mem_load += 8;
							if (!idle) {
								int offs;
								_allocate_temporary(TH::MakeSize(8, 0), &offs);
						...
						if (ld.flags & DispositionRegister) {
							...
							if (disp->flags & DispositionPointer) {
								*mem_load += _word_align(TH::MakeSize(out_size, 0));
								if (!idle) {
									int offs;
									_allocate_temporary(TH::MakeSize(out_size, 0), &offs);
					...
					if (node.self.index == TransformLogicalFork) {
						...
						if (disp->flags & DispositionRegister) {
							...
							*mem_load += 4;
							if (!idle) {
								int offs;
								_allocate_temporary(node.retval_spec.size, &offs);
								...
								if (node.self.index == TransformFollowPointer) {
									...
									if (node.inputs[1].self.ref_class == ReferenceLiteral && (node.inputs.Length() == 2 || node.inputs[2].self.ref_class == ReferenceLiteral)) {
										...
										if (!idle && offset * scale) {
											if (scale != 0xFFFFFFFF) encode_add(Reg::ESI, offset * scale);
											else encode_add(Reg::ESI, -offset);
										}
